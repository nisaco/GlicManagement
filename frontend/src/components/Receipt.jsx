...
export default function Receipt({ paymentId, onClose }) {
  ...
  const sendWhatsApp = () => {
    const num = payment.member?.whatsapp?.replace(/\D/g, '');
    if (!num) { alert('This member has no WhatsApp number on file. Please add it to their profile first.'); return; }
    const period = payment.month ? `${MONTHS[payment.month-1]} ${payment.year||''}` : '—';
    const msg =
`✝️ *Gospel Light International Church*
*Payment Receipt*

Dear ${payment.member?.firstName},

We confirm receipt of your payment:

- *Type:* ${payment.type?.replace('_',' ').replace(/\b\w/g,c=>c.toUpperCase())}
- *Amount:* GH₵ ${payment.amount?.toLocaleString()}
- *Period:* ${period}
- *Method:* ${payment.method?.replace('_',' ').replace(/\b\w/g,c=>c.toUpperCase())}
- *Reference:* ${payment.reference||'N/A'}
- *Date:* ${new Date(payment.createdAt).toLocaleDateString('en-GH',{day:'numeric',month:'long',year:'numeric'})}
      ...
      <div className="modal-box" style={{ maxWidth:460 }}>
        <div className="modal-head">
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={sendWhatsApp} style={{ padding:'7px 14px', borderRadius:9, background:'#25D366', color:'#fff', border:'none', fontSize:12.5, fontWeight:600, cursor:'pointer', fontFamily:'var(--font-body)', display:'flex', alignItems:'center', gap:5 }}>
              📱 WhatsApp
            </button>
            <button onClick={handlePrint} className="btn btn-gold btn-sm">🖨 Print</button>
            <button onClick={onClose} style={{ background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.6)', width:32, height:32, borderRadius:8, cursor:'pointer', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
          </div>
        </div>
        ...
        <div ref={printRef} style={{ padding:28 }}>
          ...
          {payment.notes && (
            <div style={{ marginTop:12, fontSize:12, color:'var(--muted)', fontStyle:'italic' }}>Note: {payment.notes}</div>
          )}
