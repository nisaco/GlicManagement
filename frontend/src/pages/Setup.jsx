...
export default function Setup() {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      await axios.post('/api/auth/setup', { name: form.name, email: form.email, password: form.password });
      setSuccess(true);
    ...
    <div style={{ minHeight:'100vh', background:'var(--navy)', display:'flex', alignItems:'center', justifyContent:'center', padding:24, position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ width:'100%', maxWidth:440, position:'relative', zIndex:2 }} className="animate-up">
        <div style={{ textAlign:'center', marginBottom:30 }}>
          <div style={{ width:62, height:62, borderRadius:18, background:'linear-gradient(135deg,var(--gold),#8B6420)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px', boxShadow:'0 0 36px rgba(201,148,58,0.3)', fontFamily:'var(--font-display)', fontSize:22, fontWeight:700, color:'var(--navy)', overflow:'hidden' }}>
            {CHURCH.logo ? <img src={CHURCH.logo} alt="logo" style={{ width:'100%', height:'100%', objectFit:'cover', borderRadius:18 }}/> : CHURCH.initials}
          </div>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', backdropFilter:'blur(20px)', borderRadius:22, border:'1px solid rgba(201,148,58,0.2)', padding:34, boxShadow:'0 24px 60px rgba(0,0,0,0.5)' }}>
          {success ? (
            <div style={{ textAlign:'center', padding:'20px 0' }}>
              ...
              <div style={{ fontFamily:'var(--font-display)', fontSize:22, fontWeight:600, color:'#fff', marginBottom:8 }}>Admin Account Created!</div>
            ...
            <>
              ...
              <p style={{ fontSize:12.5, color:'rgba(255,255,255,0.35)', marginBottom:24 }}>
                This is a one-time setup. Only you will have access to create other user accounts.
              </p>

              {error && (
                <div style={{ background:'rgba(224,85,85,0.12)', border:'1px solid rgba(224,85,85,0.25)', color:'#FCA5A5', fontSize:13, padding:'11px 14px', borderRadius:10, marginBottom:18 }}>
                  ⚠ {error}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                  {[
                    { label:'Your Full Name',   key:'name',     type:'text',     ph:'e.g. Pastor John Mensah' },
                    { label:'Email Address',    key:'email',    type:'email',    ph:'admin@glicghana.org'      },
                ...
                <button type="submit" disabled={loading} className="btn btn-gold"
                  style={{ width:'100%', padding:13, fontSize:14, borderRadius:12, marginTop:22, justifyContent:'center' }}>
