...
const SERVICE_LABELS = {
  sunday_main:   '🙏 Sunday Main',
  sunday_second: '🙏 Sunday Second',
  midweek:       '📖 Midweek',
...
export default function Attendance() {
  ...
  const fetchAll = async () => {
    ...
    setRecords(r.data); setMembers(m.data); setStats(s.data);
    setLoading(false);
  };
  useEffect(() => { fetchAll(); }, []);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      await axios.post('/api/attendance', {
        ...
        members:    form.useManual ? [] : [...present],
        totalCount,
        notes:      form.notes,
      });
      ...
      await logActivity(
        'Attendance recorded',
        'attendance',
        `${SERVICE_LABELS[form.service] || form.service} — ${new Date(form.date).toLocaleDateString('en-GH', { day:'numeric', month:'short', year:'numeric' })} · ${totalCount} people`,
        { service: form.service, date: form.date, count: totalCount }
      );

      setShowForm(false);
      setPresent(new Set());
      setManualCount('');
      setForm({ date: new Date().toISOString().slice(0,10), service:'sunday_main', notes:'', useManual:false });
      fetchAll();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14, marginBottom:24 }} className="stagger stats-4">
        {[
          { label:'Services This Month', value: stats?.thisMonthServices ?? '—', color:'var(--gold2)' },
          { label:'Avg Attendance',      value: stats?.avgAttendance     ?? '—', color:'var(--blue)'  },
          { label:'Last Service',        value: stats?.lastServiceCount  ?? '—', color:'var(--green)', sub: stats?.lastServiceDate ? new Date(stats.lastServiceDate).toLocaleDateString('en-GH') : '' },
        ].map(s => (
          <div key={s.label} className="glass-card animate-up" style={{ padding:'20px 22px', position:'relative', overflow:'hidden' }}>
          ...
          <table className="data-table">
            ...
            <tbody>
              ...
              {!loading && records.map(r => (
                <tr key={r._id}>
                  <td style={{ fontWeight:500 }}>
                    {new Date(r.date).toLocaleDateString('en-GH', { weekday:'short', day:'numeric', month:'short', year:'numeric' })}
                  </td>
                  <td style={{ color:'rgba(255,255,255,0.7)' }}>{SERVICE_LABELS[r.service] || r.service}</td>
                  <td>
                    <span style={{ fontFamily:'var(--font-display)', fontSize:22, fontWeight:700, color:'var(--gold2)' }}>{r.totalCount}</span>
                    <span style={{ fontSize:12, color:'var(--muted)', marginLeft:4 }}>people</span>
                  </td>
              ...
              {!loading && !records.length && (
                <tr><td colSpan={5} style={{ padding:36, textAlign:'center', color:'var(--muted)' }}>No records yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            <div className="modal-body">
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ display:'flex', gap:8, marginBottom:16 }}>
                  {[
                    { k:false, label:'✅ Tick Members'    },
                    { k:true,  label:'🔢 Enter Count Only' },
                  ].map(o => (
                    <button type="button" key={String(o.k)}
                      onClick={() => setForm(f => ({...f, useManual:o.k}))}
                      style={{ flex:1, padding:10, borderRadius:10, fontSize:13, cursor:'pointer',
                ...
                {form.useManual ? (
                  <div style={{ marginBottom:16 }}>
                    ...
                    <input type="number" value={manualCount} onChange={e => setManualCount(e.target.value)}
                      required min="1" placeholder="e.g. 120" className="field-input"/>
                  </div>
                ) : (
                  <div style={{ marginBottom:16 }}>
                    ...
                    <div style={{ border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, maxHeight:240, overflowY:'auto', background:'rgba(255,255,255,0.02)' }}>
                      {members.map(m => (
                        <div key={m._id} onClick={() => toggle(m._id)}
                          style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderBottom:'1px solid rgba(255,255,255,0.04)', cursor:'pointer', transition:'background 0.15s',
                            background: present.has(m._id) ? 'rgba(201,148,58,0.07)' : 'transparent' }}>
                          <div style={{ width:17, height:17, borderRadius:5, flexShrink:0,
                            border:     `1.5px solid ${present.has(m._id) ? 'var(--gold)' : 'rgba(255,255,255,0.2)'}`,
                            background: present.has(m._id) ? 'var(--gold)' : 'transparent',
                          ...
                          <span style={{ fontSize:13, color:present.has(m._id) ? 'var(--white)' : 'rgba(255,255,255,0.6)', flex:1 }}>
                            {m.firstName} {m.lastName}
                          </span>
                          <span style={{ fontSize:11, color:'var(--muted)', textTransform:'capitalize' }}>{m.role}</span>
                        </div>
                      ))}
                      {!members.length && (
                        <div style={{ padding:20, textAlign:'center', color:'var(--muted)', fontSize:13 }}>No active members found.</div>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div style={{ marginBottom:16 }}>
                  ...
                  <input type="text" value={form.notes} onChange={e => setForm(f => ({...f, notes:e.target.value}))}
                    placeholder="e.g. Harvest Sunday, Guest preacher..." className="field-input"/>
                </div>
