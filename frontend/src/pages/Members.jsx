...
const emptyForm = { firstName:'',lastName:'',email:'',phone:'',whatsapp:'',dob:'',membershipDate:'',baptismDate:'',status:'active',role:'member',departments:[],household:'',duesAmount:200,notes:'' };
...
export default function Members() {
  ...
  const openEdit = m => {
    setForm({ ...m, dob:m.dob?.slice(0,10)||'', membershipDate:m.membershipDate?.slice(0,10)||'', baptismDate:m.baptismDate?.slice(0,10)||'' });
  ...
  const toggleDept = dept => setForm(f => ({ ...f, departments: f.departments.includes(dept) ? f.departments.filter(d=>d!==dept) : [...f.departments,dept] }));
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:18, flexWrap:'wrap', alignItems:'center' }}>
        <input placeholder="Search name, phone, email..." value={search} onChange={e=>setSearch(e.target.value)} className="field-input" style={{ maxWidth:260, padding:'9px 14px' }}/>
        {['all','active','inactive','visitor'].map(f => (
          <button key={f} onClick={()=>setFilter(f)} style={{ padding:'7px 16px', borderRadius:20, fontSize:12.5, cursor:'pointer', fontFamily:'var(--font-body)', transition:'all 0.18s', border:'none',
      ...
      <div className="glass-card">
        <div className="table-wrap">
          <table className="data-table">
            <thead>
              <tr>
                {['Member','WhatsApp','Role','Departments','Monthly Dues','Status',''].map(h => <th key={h}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={7} style={{ padding:28, textAlign:'center', color:'var(--muted)' }}><div className="spinner"/></td></tr>}
              {!loading && members.map(m => (
                <tr key={m._id}>
                  <td>
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      <div style={{ width:36,height:36,borderRadius:10,background:'rgba(201,148,58,0.12)',border:'1px solid rgba(201,148,58,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'var(--gold2)',flexShrink:0 }}>
              ...
              {!loading && !members.length && (
                <tr><td colSpan={7} style={{ padding:36, textAlign:'center', color:'var(--muted)' }}>No members found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            <div className="modal-body">
              <form onSubmit={handleSubmit}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                  ...
                  <F label="Household / Family"  k="household"      form={form} setForm={setForm} />
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginTop:14 }}>
                  <div>
                    ...
                    <select value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))} className="field-input">
                      {['member','deacon','elder','pastor','youth','visitor'].map(r=><option key={r} value={r}>{r.charAt(0).toUpperCase()+r.slice(1)}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="field-label">Status</label>
                    <select value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))} className="field-input">
                      {['active','inactive','visitor'].map(s=><option key={s} value={s}>{s.charAt(0).toUpperCase()+s.slice(1)}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <div style={{ marginTop:14 }}>
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginTop:2 }}>
                    {DEPARTMENTS.map(d => (
                      <button type="button" key={d} onClick={()=>toggleDept(d)} style={{ padding:'6px 14px', borderRadius:20, fontSize:12.5, cursor:'pointer', fontFamily:'var(--font-body)', transition:'all 0.18s',
                ...
                <div style={{ marginTop:14 }}>
                  ...
                  <textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} className="field-input" style={{ resize:'vertical', minHeight:70 }} placeholder="Any additional notes..."/>
                </div>

                <div className="modal-foot" style={{ padding:'18px 0 0', border:'none', justifyContent:'flex-end', display:'flex', gap:10 }}>
...
const F = ({ label, k, form, setForm, type='text', ph, required }) => (
  <div>
    ...
    <input type={type} value={form[k]} placeholder={ph||''} required={required} className="field-input"
